...
// Transport is the bidirectional message channel between the LSP client
// and the server. Messages are framed with `Content-Length` headers, the
// same convention used by DAP.
...
// ReadMessage reads one framed message and returns its raw JSON payload.
