	...
	for _, line := range lines {
		...
		default:
			...
			result, stdoutResult = captureStdout(func() (types.MalType, error) {
				v, err := REPL(ctx, env, line, types.NewCursorFile(fileName), nil)
				if v == nil {
					return "nil", err
				}
				return v, err
...
func newEnv(fileName string) types.EnvType {
	...
	if _, err := REPL(ctx, newenv, `(eval (read-string (str "(do "`+future+`" nil)")))`, types.NewCursorFile(reflect.TypeOf(&future).PkgPath()), nil); err != nil {
		return nil
	}

	// core.mal: defined using the language itself
	if _, err := REPL(ctx, newenv, `(def *host-language* "go")`, types.NewCursorFile(fileName), nil); err != nil {
		return nil
	}
	if _, err := REPL(ctx, newenv, "(def not (fn (a) (if a false true)))", types.NewCursorFile(fileName), nil); err != nil {
		return nil
	}
	if _, err := REPL(ctx, newenv, `(def load-file (fn (f) (eval (read-string (str "(do " (slurp f) "\nnil)")))))`, types.NewCursorFile(fileName), nil); err != nil {
		return nil
	}
	if _, err := REPL(ctx, newenv, "(defmacro cond (fn (& xs) (if (> (count xs) 0) (list 'if (first xs) (if (> (count xs) 1) (nth xs 1) (throw \"odd number of forms to cond\")) (cons 'cond (rest (rest xs)))))))", types.NewCursorFile(fileName), nil); err != nil {
		return nil
	}
	return newenv
}
