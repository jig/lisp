...
Signing is **per-call and declared by the code**: `:sign` on `git-commit` / annotated `git-tag` carries an allowed-signers list (authorized_keys-format **public** keys), and the runtime signs with a matching key from the ssh-agent — or from a signer a Go embedder registered with `SetSigner` (e.g. a `crypto.Signer` adapted via `gossh.NewSignerFromSigner`). No `:sign`, no signature; there is no process-global switch, and these keys are usually different from `/etc/lisp/allowed_signers`, which verifies *code* and never signs anything. The agent path signs with the **ssh-agent** key whose public key is listed in FILE (the same trusted-key list that verifies the code ref). The private key never enters the process — it stays in the agent, unlocked however the agent is (keychain, `ssh-add`, …). No policy → commits and tags are unsigned.
