...
### ⚠️ Changed — radix literals are unsigned big-int data numbers

`0x…`, `0o…` and `0b…` literals now read as **arbitrary-precision,
unsigned data numbers** (`*big.Int`) — the natural type for serial
numbers, hashes and masks — and print back as `0x…` uppercase hex
padded to whole octets (`0xf` → `0x0F`, zero → `0x00`), so they
round-trip through the printer. They compare numerically with `=`
against other big ints, but are **distinct from machine ints**
(`(= 0x0A 10)` is `false`) and do not participate in arithmetic
(`(+ 0x01 1)` errors). A sign is not part of a data number: `-0x01` is
a read error. Decimal literals keep reading as signed machine ints,
unchanged; legacy leading-zero octal (`042`, `00`) is error-prone and
now a read error too — use `0o…`.
