...
func eval_ast(ctx context.Context, ast MalType, env EnvType, debug Debug) (MalType, error) {
	if Q[Symbol](ast) {
...
func do(ctx context.Context, ast MalType, from, to int, env EnvType, debug Debug) (MalType, error) {
	if ast == nil {
		return nil, nil
	}
	lst := ast.(List).Val
	...
	evaledAST, e := eval_ast(ctx, List{Val: lst[from : len(lst)+to]}, env, debug)
	if e != nil {
		return nil, e
	}
...
func EVAL(ctx context.Context, ast MalType, env EnvType, debug Debug) (res MalType, e error) {
	for {
		if ctx != nil {
			select {
			case <-ctx.Done():
		...
		default:
			// aStr, _ := PRINT(ast)
			// fmt.Printf("%T○ %s\n", ast, aStr)
			return eval_ast(ctx, ast, env, debug)
		}
		...
		case "let":
			...
			for i := 0; i < len(arr1); i += 2 {
				...
				exp, e := EVAL(ctx, arr1[i+1], let_env, debug)
				if e != nil {
					return nil, e
				}
			...
			astRef := ast.(List)
			ast, e = do(ctx, astRef, 2, -1, let_env, debug)
			if e != nil {
				return nil, e
			}
			env = let_env
		...
		case "try":
			...
			exp, e := func() (res MalType, err error) {
				...
				if dl, ok := ctx.Deadline(); ok {
					...
					defer cancel()
					return do(ctx, tryDo, 0, 0, env, debug)
				}
				return do(ctx, tryDo, 0, 0, env, debug)
			}()

			defer func() { _, _ = do(ctx, finallyDo, 0, 0, env, debug) }()
			...
			if e == nil {
				...
				if catchDo != nil {
					...
					ast, err = do(ctx, catchDo, 0, 0, new_env, debug)
					if err != nil {
						return nil, err
					}
					env = new_env
		...
		case "do":
			var err error
			ast, err = do(ctx, ast, 1, -1, env, debug)
			if err != nil {
				return nil, err
			}
		case "if":
			cond, e := EVAL(ctx, a1, env, debug)
			if e != nil {
				return nil, e
			}
		...
		case "fn":
			...
			return fn, nil
		default:
			el, e := eval_ast(ctx, ast, env, debug)
			if e != nil {
				return nil, e
			}
			f := el.(List).Val[0]
...
func REPL(ctx context.Context, env EnvType, sourceCode string, cursor *Position, debug Debug) (MalType, error) {
	ast, err := READ(sourceCode, cursor, env)
	if err != nil {
		return nil, err
	}
	exp, err := EVAL(ctx, ast, env, debug)
	if err != nil {
		return nil, err
	}
...
func REPLWithPreamble(ctx context.Context, env EnvType, sourceCode string, cursor *Position, debug Debug) (MalType, error) {
...
func ReadEvalWithPreamble(ctx context.Context, env EnvType, sourceCode string, cursor *Position, debug Debug) (MalType, error) {
	ast, err := READWithPreamble(sourceCode, cursor, env)
	if err != nil {
		return nil, err
	}
