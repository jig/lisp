...
func macroexpand(ctx context.Context, ast MalType, env EnvType, dbg debug.Debug) (MalType, error) {
	var mac MalType
	var e error
...
func eval_ast(ctx context.Context, ast MalType, env EnvType, dbg debug.Debug) (MalType, error) {
	if Q[Symbol](ast) {
...
func do(ctx context.Context, ast MalType, from, to int, env EnvType, dbg debug.Debug) (MalType, error) {
	if ast == nil {
		return nil, nil
	}
	lst := ast.(List).Val
	...
	evaledAST, e := eval_ast(ctx, List{Val: lst[from : len(lst)+to]}, env, dbg)
	if e != nil {
		return nil, e
	}
...
// Eval evaluates a Lisp AST value in the given environment context with optional debugging support.
// It returns the evaluated result and any error that occurred during evaluation.
//
// Parameters:
//   - ctx: The context for cancellation and timeouts
//   - ast: The AST value to evaluate (a list representing a Lisp expression)
//   - env: The environment containing variable bindings and function definitions
//   - debug: Optional debug handler for step-by-step execution control (can be nil)
//
// Returns:
//   - Value: The result of the evaluation in a lisp.Value (printable with lisp.PrintString)
//   - error: An error if the evaluation fails
func EVAL(ctx context.Context, ast MalType, env EnvType, dbg debug.Debug) (result MalType, err error) {
	// if no debug set we proceed with normal evaluation
	if dbg == nil {
		if _, ok := ast.(List); !ok {
			return eval_ast(ctx, ast, env, nil)
		}
		return eval(ctx, ast, env, nil)
	}

	// if we are cancelling evaluation we exit immediately
	if dbg.CancelStatus() {
		return nil, nil
	}

	// debug normal evaluation starts here
	if _, ok := ast.(List); !ok {
		return eval_ast(ctx, ast, env, dbg)
	}

	// if step-by-step is disabled we just eval, but we still need to pass debug object
	// to allow it to handle the debugged files/strings map via debug.PushFile
	if dbg.DoNotStopStatus() {
		return eval(ctx, ast, env, dbg)
	}

	// if step-by-step is disabled we grab current evaluation state and wait for the debug control command
	switch dbg.Wait(debug.DebugMessage{
		Env:   env,
		Input: &ast,
	}) {
	case debug.DebugStepInto:
		result, err = eval(ctx, ast, env, dbg)
	case debug.DebugStepOver:
		previously := dbg.SetDoNotStop(true)
		defer dbg.SetDoNotStop(previously)

		result, err = eval(ctx, ast, env, dbg)
	case debug.DebugStepOut:
		previously := dbg.SetDoNotStop(true)
		defer dbg.SetDoNotStop(previously)
	...
	case debug.DebugContinue:
		dbg.SetDoNotStop(true)
		return eval(ctx, ast, env, dbg)
	case debug.DebugNoop:
		panic("DebugNoop is not a valid control command")
	default:
		panic("Unknown debug control command")
	}
	...
	case debug.DebugStepInto, debug.DebugStepOver, debug.DebugStepOut, debug.DebugContinue:
	case debug.DebugExit:
		return nil, nil
	case debug.DebugNoop:
		panic("DebugNoop is not a valid control command")
...
func eval(ctx context.Context, ast MalType, env EnvType, dbg debug.Debug) (res MalType, e error) {
	for {
		if ctx != nil {
			select {
			case <-ctx.Done():
		...
		// apply list
		ast, e = macroexpand(ctx, ast, env, dbg)
		if e != nil {
			return nil, e
		}
		if !Q[List](ast) {
		...
		case "let":
			...
			for i := 0; i < len(arr1); i += 2 {
				...
				exp, e := EVAL(ctx, arr1[i+1], let_env, dbg)
				if e != nil {
					return nil, e
				}
			...
			astRef := ast.(List)
			ast, e = do(ctx, astRef, 2, -1, let_env, dbg)
			if e != nil {
				return nil, e
			}
			env = let_env
		...
		case "macroexpand":
			return macroexpand(ctx, a1, env, dbg)
		case "try":
			lst := ast.(List).Val
			var last MalType
			var prelast MalType
			...
			exp, e := func() (res MalType, err error) {
				...
				if dl, ok := ctx.Deadline(); ok {
					...
					defer cancel()
					return do(ctx, tryDo, 0, 0, env, dbg)
				}
				return do(ctx, tryDo, 0, 0, env, dbg)
			}()

			defer func() { _, _ = do(ctx, finallyDo, 0, 0, env, dbg) }()
			...
			if e == nil {
				...
				if catchDo != nil {
					...
					ast, err = do(ctx, catchDo, 0, 0, new_env, dbg)
					if err != nil {
						return nil, err
					}
					env = new_env
		...
		case "do":
			var err error
			ast, err = do(ctx, ast, 1, -1, env, dbg)
			if err != nil {
				return nil, err
			}
		case "if":
			cond, e := EVAL(ctx, a1, env, dbg)
			if e != nil {
				return nil, e
			}
		...
		case "fn":
			fn := MalFunc{
				Eval: func(ctx context.Context, ast MalType, env EnvType, dbg any) (res MalType, e error) {
					if dbg != nil {
						return EVAL(ctx, ast, env, dbg.(debug.Debug))
					}
					return EVAL(ctx, ast, env, nil)
				},
		...
		default:
			...
			if Q[MalFunc](f) {
				...
				result, err := fn.Fn(ctx, dbg, el.(List).Val[1:])
				if err != nil {
					return nil, lisperror.NewLispError(err, ast)
				}
				return result, nil
...
func REPL(ctx context.Context, env EnvType, sourceCode string, cursor *Position, dbg debug.Debug) (MalType, error) {
	ast, err := READ(sourceCode, cursor, env)
	if err != nil {
		return nil, err
	}
	exp, err := EVAL(ctx, ast, env, dbg)
	if err != nil {
		return nil, err
	}
...
func REPLWithPreamble(ctx context.Context, env EnvType, sourceCode string, cursor *Position, dbg debug.Debug) (MalType, error) {
...
func ReadEvalWithPreamble(ctx context.Context, env EnvType, sourceCode string, cursor *Position, dbg debug.Debug) (MalType, error) {
	ast, err := READWithPreamble(sourceCode, cursor, env)
	if err != nil {
		return nil, err
	}
