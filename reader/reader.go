...
func read_atom(rdr Reader) (MalType, error) {
	...
	if tokenStruct == nil {
		return nil, lisperror.NewMalError(errors.New("read_atom underflow"), &tokenStruct)
	}
	...
	if match := integerRE.MatchString(*token); match {
		...
		if i, e = strconv.Atoi(*token); e != nil {
			return nil, lisperror.NewMalError(errors.New("number parse error"), &tokenStruct)
		}
		return i, nil
		...
		return nil, lisperror.NewMalError(errors.New("expected '\"', got EOF"), &tokenStruct)
...
func read_list(rdr Reader, start string, end string, placeholderValues *HashMap) (MalType, error) {
	tokenStruct := rdr.next()
	if tokenStruct == nil {
		return nil, lisperror.NewMalError(errors.New("read_list underflow"), &tokenStruct)
	}
	cursor := tokenStruct.Cursor.Copy()
	token := &tokenStruct.Value
	if *token != start {
		return nil, lisperror.NewMalError(errors.New("expected '"+start+"'"), &tokenStruct)
	}
	...
	for ; true; tokenStruct = rdr.peek() {
		if tokenStruct == nil {
			return nil, lisperror.NewMalError(errors.New("expected '"+end+"', got EOF"), lastKnown)
		}
		lastKnown = tokenStruct
		token = &tokenStruct.Value
		if *token == end {
...
func read_placeholder(rdr Reader, placeholderValues *HashMap) (MalType, error) {
	tokenStruct := rdr.next()
	if tokenStruct == nil {
		return nil, lisperror.NewMalError(errors.New("read_placeholder underflow"), &tokenStruct)
	}
...
func read_form(rdr Reader, placeholderValues *HashMap) (MalType, error) {
	tokenStruct := rdr.peek()
	if tokenStruct == nil {
		return nil, lisperror.NewMalError(errors.New("read_form underflow"), &tokenStruct)
	}
	cursor := tokenStruct.Cursor.Copy()
	...
	// list
	case ")":
		return nil, lisperror.NewMalError(errors.New("unexpected ')'"), &tokenStruct)
	case "(":
...
func Read_str(str string, cursor *Position, placeholderValues *HashMap) (MalType, error) {
	...
	if tokenReader.position != len(tokenReader.tokens) {
		return nil, lisperror.NewMalError(errors.New("not all tokens where parsed"), cursor)
	}
	return res, nil
}
